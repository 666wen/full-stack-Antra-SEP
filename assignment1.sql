...
-- 1. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.
-- Product table, with no filter. 

SELECT ProductID, Name, Color, ListPrice
FROM Production.Product

-- 2. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.
...
-- by ProductID from 400 to 500 using between
...
-- columns ProductSubcategoryID and Color from the Production.Product table. We do not want any rows that 
-- are NULL.in any of the two columns in the result. (Hint: Use IsNull)
SELECT DISTINCT ISNULL(ProductSubcategoryID, 0), ISNULL(Color, 'aaa') 
FROM Production.Product